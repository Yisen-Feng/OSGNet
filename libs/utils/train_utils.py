...
from basic_utils import save_json,get_memory
import pkg_resources


from mamba_ssm.ops.triton.layernorm import RMSNorm as RMSNormGated
################################################################################
...
def make_optimizer(model, optimizer_config, head_backbone_group=False):
    ...
    # whitelist_weight_modules = (torch.nn.Linear, torch.nn.Conv1d, MaskedConv1D,torch.nn.NonDynamicallyQuantizableLinear,torch.nn.MultiheadAttention,torch.nn.Embedding)
    whitelist_weight_modules = (torch.nn.Linear, torch.nn.Conv1d, MaskedConv1D,torch.nn.MultiheadAttention,torch.nn.Embedding)
    blacklist_weight_modules = (LayerNorm, torch.nn.GroupNorm,torch.nn.LayerNorm,LayerNorm4,RMSNormGated,T5LayerNorm)
...
def train_one_epoch(
    ...
    for iter_idx, video_list in tqdm.tqdm(enumerate(train_loader, 0), desc="training one epoch"):
        ...
        t=(curr_epoch*num_iters+(1+iter_idx))/scheduler.max_epochs
        ...
        #     losses = model(video_list,t=t)
        # except:
        #     from basic_utils import save_model_and_data
        #     save_model_and_data(model,optimizer,curr_epoch,video_list,'./debug.ckpt')
        #     exit()
...
def valid_one_epoch_nlq_singlegpu(
    ...
    for iter_idx, video_list in enumerate(val_loader, 0):
        with torch.no_grad():
            ...
            #     print(item)
            output = model(video_list,mode=mode,t=t)
            # print("output, ", output)
