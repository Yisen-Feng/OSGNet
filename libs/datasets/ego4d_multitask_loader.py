...
@register_dataset("ego4d_multitask")
class Ego4dMultitaskDataset(Dataset):
    def __init__(
            self,
            ...
            text_feat_with_eot=False,
            object_feat_dir=None,
            classname_feat_dir=None,
            classname_feat_concat=None,
            top5_object_feat_dir=None,
            object_feat_type=None,
            lavila_caption_dir=None,
            max_shot_second=86400,
            use_Bayesian=False,
            split_word_list=['looks around' , 'turns around' ],
            mul_query_input=1000,
            narration_jsonl_file=None,
    ):
        self.max_shot_second=max_shot_second
        self.use_Bayesian=use_Bayesian
        self.split_word_list=split_word_list
        self.text_feature_dict={}
        self.mul_query_input=mul_query_input
        self.narration_jsonl_file=narration_jsonl_file
        # file path
        ...
        self.text_feat_with_eot=text_feat_with_eot
        # split / training mode
        self.split = split
        ...
        self.lavila_caption_dir=lavila_caption_dir
    ...
    def load_data(self):
        ...
        group_vid_data={key:list(group) for key , group in groupby(datalist,key=lambda x: x['video_id'])}
        self.vid_dict={}
        for vid,vid_data in group_vid_data.items():
            vid_item={'query_ids':[],'timestamps':[]}
            for vid_d in vid_data:
                vid_item['query_ids'].append(vid_d['query_id'])
                if 'test' not in self.split:
                    vid_item['timestamps'].append(vid_d['timestamps'])
                # vid_item['timestamps'].append(vid_d['timestamps'])
            self.vid_dict[vid]=vid_item
        if self.narration_jsonl_file is not None:
            narrationlist=load_jsonl(self.narration_jsonl_file)
            group_vid_narration_data={key:list(group) for key , group in groupby(narrationlist,key=lambda x: x['video_id'])}
            for vid,vid_data in group_vid_narration_data.items():
                vid_item={'query_ids':[],'timestamps':[]}
                for vid_d in vid_data:
                    vid_item['query_ids'].append(vid_d['query_id'])
                    if 'test' not in self.split:
    ...
    def __getitem__(self, idx):
        ...
        if task_name not in ["narration", "nlq","nlq_narration","goal_step",'tacos']:
            print("unsupported task name: ", task_name)
            exit(1)
        # convert time stamp (in second) into temporal feature grids
        # ok to have small negative values here
        if 'timestamps' in video_item.keys():
            temp_timestamps=self.get_timestamps(video_item['timestamps'])
        ...
        if self.lavila_caption_dir is not None:
            file_path=os.path.join(self.lavila_caption_dir,'{}.json'.format(video_item['video_id']))
            lavila_caption=load_jsonl(file_path)
            shot_frame=[]
            for lavila_cap in lavila_caption:
                lavila_cap_narrow=lavila_cap['narration'].lower()
                lavila_cap_nar=lavila_cap_narrow.split(';')
                for lavila_cap_n in lavila_cap_nar:
                    frame_id=lavila_cap['frame_start_id']
                    if len(shot_frame)>0 and frame_id >=shot_frame[-1]+self.max_shot_second*self.fps_attributes["default_fps"]:
                        shot_frame.append(frame_id)
                        break
                    if '#c' in lavila_cap_n:
                        for word in self.split_word_list:
                        # for word in ['looks around' , 'turns around' , 'walks' , 'moves around',' rides' ,' runs',' cycles',' jogs',' jumps']:
                        # for word in ['looks around' , 'turns around' , 'walks' , 'moves around']:
                            if word in lavila_cap_n :
                    # if '#c' in lavila_cap_n and ('looks around' in lavila_cap_n or 'turns around' in lavila_cap_n):
                                shot_frame.append(lavila_cap['frame_start_id'])
                                break
                        if (len(self.split_word_list)==0) or (word in lavila_cap_n) :
                            break
            shot_left=torch.tensor(shot_frame,dtype=torch.int)/self.fps_attributes["num_frames"]
            if shot_left.shape[0]==0 or shot_left[0]>0:
                shot_left=torch.cat([torch.tensor([0]),shot_left],dim=0)
            shot_left=shot_left[shot_left<vid_len]#该死的，我忘了naq的特征长度是错的
            shot_left=torch.cat([shot_left,torch.tensor([vid_len])],dim=0)
            shot_right=torch.cat([shot_left[1:],torch.tensor([self.max_seq_len])],dim=0)
            # shots_len=shot_right-shot_left
            shots=torch.stack([shot_left,shot_right],dim=0)#左闭右开
            data_dict.update({
            'shots': shots,  # 2xN 
            
            })
        query_feat = self._get_query_feat_by_qid(self.clip_textual_txn,real_query_id)
        if self.text_feat_with_eot:
        ...
        if self.mul_query_input>0:
            query_ids=self.vid_dict[video_item['video_id']]['query_ids']
            if len(query_ids)>self.mul_query_input:
                indices = random.sample(range(len(query_ids)), self.mul_query_input)
                query_ids=[query_ids[i] for i in indices]
            query_feats=[]
            for query_id in query_ids:
                query_feat = self._get_query_feat_by_qid(self.clip_textual_txn,query_id)
                query_feat = torch.from_numpy(np.ascontiguousarray(query_feat.transpose(0, 1)))
                query_feats.append(query_feat)
            data_dict.update({
                'video_query_feats': query_feats,
                
            })
        if self.use_Bayesian:
            data_dict.update({
            
            'video_query_nums':len(query_feats)
        })
        # if self.is_training:
        if 'test' not in self.split:
            video_timestamps=self.vid_dict[video_item['video_id']]['timestamps']
            if len(video_timestamps)>self.mul_query_input:
                video_timestamps=[video_timestamps[i] for i in indices]
            segments=[]
            for video_timestamp in video_timestamps:
                temp_timestamps=self.get_timestamps(video_timestamp)
                timestamps = np.array(temp_timestamps)
                if len(timestamps.shape) == 1:
                    timestamps = timestamps.reshape(1, -1)
        ...
        for frame_objs in video_objs:
            ...
            for class_id in class_ids:
                if isinstance(class_id,int):
                    if class_id in frame_objs.keys():
                        frame_obj_collect.append(class_id)
                elif isinstance(class_id,list):
                    for class_i in class_id:
                        if class_i in frame_objs.keys():
                            frame_obj_collect.append(class_i)
                            break#只取最大的那个
                else:
                    raise ValueError('not support type')
            frame_obj_collect=list(set(frame_obj_collect))
            frame_obj_feature=classname_feat[frame_obj_collect]
            # for obj in frame_objs.keys():
            #     if obj in class_ids:
            #         frame_obj_feature.append(classname_feat[obj])#[c]
            if frame_obj_feature.shape[0]<=0:
    ...
    def _get_query_feat_by_qid(self,clip_textual_txn, qid):
        if qid in self.text_feature_dict:
            q_feat=self.text_feature_dict[qid]
        ...
        self.text_feature_dict[qid]=q_feat
        return torch.from_numpy(q_feat)  # (Lq, D), (D, )
