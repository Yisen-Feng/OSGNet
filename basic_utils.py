...
def get_memory(str):
    ...
    return
def save_model_and_data(model, optimizer, epoch, batch_data, filepath):
    # 保存模型和优化器的状态
    torch.save({
        'epoch': epoch,
    ...
    print(f'Model and data saved to {filepath}')
    return
...
def print_bar(data,title,xlabel,ylabel):
    ...
    counts = list(sorted(data,reverse=True))
    ...
    plt.show()
    return
def get_files_in_directories(folders):
    files = []
